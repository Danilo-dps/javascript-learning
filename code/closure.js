...
/*
 * Em Java, para fazer algo parecido com closure, você usaria:
 * 
 ...
 * c1.incrementar(); // 2
 * 
 * A closure em JS faz a mesma coisa, mas com menos código
 * e sem precisar de classes! É mais leve e direto.
 */

function criarContador() {
    let count = 0; // "Atributo privado" da closure
    
    // Retornamos um objeto com múltiplas funções
    return {
 ...
 * Em JS, closures permitem o mesmo padrão!
 */
...
function criarPessoa(nome, idade) {
    ...
    return {
        ...
        // Método que usa os dados privados
        apresentar: function() {
            return `Olá, eu sou ${_nome} e tenho ${_idade} anos.`;
        },
        // Método que não pode ser acessado externamente (privado)
        // Mas pode ser usado internamente
...
console.log('pessoa1._nome:', pessoa1._nome); // undefined (não existe!)
...
// Problema clássico: closure captura a referência, não o valor
console.log('PROBLEMA: closures capturam a referência');
...
function criarFuncoesProblema() {
    ...
    for (var i = 0; i < 3; i++) {
        funcoes.push(function() {
            return i; // Captura a referência de 'i'
        });
    }
...
console.log('Todas retornam 3 porque capturam a MESMA referência de i!');

// Solução 1: IIFE (Immediately Invoked Function Expression)
console.log('\nSOLUÇÃO 1: IIFE (cria um novo escopo)');

function criarFuncoesSolucao1() {
    const funcoes = [];
    
    for (var i = 0; i < 3; i++) {
        // IIFE cria um novo escopo para cada iteração
        (function(valor) {
            funcoes.push(function() {
                return valor;
            });
...
function criarFuncoesSolucao2() {
    ...
    for (let i = 0; i < 3; i++) {
        // let cria um novo escopo para cada iteração
        funcoes.push(function() {
            return i;
        });
    }
...
function criarBotoes() {
    ...
    for (let i = 1; i <= 3; i++) {
        // Cada botão "lembra" seu próprio número
        const botao = {
            id: i,
...
function criarCache() {
    ...
    return {
        ...
        getStats: function() {
            const taxaAcerto = totalAcessos > 0 
                ? ((totalCacheHits / totalAcessos) * 100).toFixed(2) 
                : 0;
            return {
                totalAcessos,
                totalCacheHits,
...
console.log('6. Cuidado com loops - use let ou IIFE');

console.log('\n🔑 DIFERENÇA CHAVE DO JAVA:');
console.log('Java: captura valor final (effectively final)');
console.log('JS: captura referência viva (pode mutar)');
console.log('Isso permite padrões que em Java exigiriam classes!');

console.log('\n✅ QUANDO USAR CLOSURES:');
console.log('- Para criar funções com "configurações" fixas');
console.log('- Para esconder dados privados (encapsulamento)');
console.log('- Em callbacks que precisam de contexto');
console.log('- Em factories e funções de alta ordem');
